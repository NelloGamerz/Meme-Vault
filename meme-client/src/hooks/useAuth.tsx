...
//         try {
//           await useUserStore.getState().fetchUserProfile(response.data.username);
//         } catch (profileError) {
...
import { useUserStore } from "../store/useUserStore"
import { useWebSocketConnectionStore } from "../store/useWebSocketConnectionStore"
import { setInStorage, removeFromStorage } from "../utils/localStorageCache"
...
const useAuth = () => {
  ...
  const login = async (data: LoginData) => {
    try {
      ...
      if (response.data && response.data.username) {
        try {
          // Properly type the store state to include the fetchUserProfile method
          const { fetchUserProfile } = useUserStore.getState() as {
            fetchUserProfile: (username: string) => Promise<void>;
          };
          await fetchUserProfile(response.data.username);
        } catch (profileError) {
          console.error("Error fetching initial user profile:", profileError);
        }
      }
