...
// // Using the shared mapApiMemeToMeme function from utils/memeMappers.ts

// // Import the localStorage cache utility
// import { getCurrentUser, removeFromStorage, setInStorage } from "../utils/localStorageCache";
...
//   connectWebSocket: () => {
//     // Use the centralized WebSocketConnectionStore instead of implementing connection logic here
//     const { useWebSocketConnectionStore } = require("./useWebSocketConnectionStore");
//     useWebSocketConnectionStore.getState().connectWebSocket();
    
//     // Register our message handlers after connection is established
//     const wsStore = useWebSocketStore.getState();
//     if (wsStore.isConnected) {
//       console.log("WebSocket already connected, setting up handlers");
...
//       }
//     } else {
//       // Set up a listener for when the connection is established
//       const unsubscribe = useWebSocketStore.subscribe((state) => {
//         if (state.isConnected) {
//           console.log("WebSocket connection established, setting up handlers");
//           registerMessageHandlers();
...
//             // Check if this is the current user's action
//             const isCurrentUserAction = data.userId === user.userId;
            
//             // If this is the current user's action, update the liked status based on the action
//             if (isCurrentUserAction && data.action !== undefined) {
//               const isCurrentlyLiked = state.likedMemes.some((m) => m.id === data.memeId);
...
//                   likedMemes = state.likedMemes.filter((m) => m.id !== data.memeId);
//                 }
//               }
//             } else if (data.likedUserIds && user.username) {
//               // If we have likedUserIds, use that to determine if the meme is liked
//               const memeObj =
//                 state.memes.find((m) => m.id === data.memeId) ||
//                 state.userMemes.find((m) => m.id === data.memeId) ||
//                 state.savedMemes.find((m) => m.id === data.memeId);
...
//             // If this is the current user's action, update the saved status based on the action
//             if (isCurrentUserAction && data.action !== undefined) {
//               const isCurrentlySaved = state.savedMemes.some((m) => m.id === data.memeId);
...
//                     state.memes.find((m) => m.id === data.memeId) ||
//                     state.userMemes.find((m) => m.id === data.memeId) ||
//                     state.likedMemes.find((m) => m.id === data.memeId);
...
//                   savedMemes = state.savedMemes.filter((m) => m.id !== data.memeId);
//                 }
//               }
//             } else if (data.savedUserIds && user.username) {
//               // If we have savedUserIds, use that to determine if the meme is saved
...
//     if (wsStore.isConnected) {
//       // Send leave request but don't close the connection
//       wsStore.sendLeavePostRequest(postId);

//       if (currentMemeId === postId) {
//         currentMemeId = null;
//         // Keep lastJoinedPostId for potential reconnection
//         // This ensures we can rejoin the post if needed
//       }
//     }
//   },
...
//     console.log('MemeStore: Updating UI with comment:', comment);
    
//     set((state) => {
//       const updateMemeInArray = (memes: Meme[]): Meme[] =>
//         memes.map((meme) => {
//           if (meme.id === comment.memeId) {
//             console.log(`MemeStore: Updating meme ${meme.id} with new comment`);
...
//             if (commentExists) {
//               console.log('MemeStore: Comment already exists, skipping update');
...
//             console.log(`MemeStore: Meme ${meme.id} comments updated: ${updatedComments.length} comments`);
...
//       console.log('MemeStore: State updated with new comment');
//       return updatedState;
//     });
//   }
// }));