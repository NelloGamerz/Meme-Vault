...
export const ExplorePage = () => {
  ...
  const renderSkeletonCards = () => {
    return Array.from({ length: 12 }, (_, index) => <SkeletonCard key={`skeleton-${index}`} index={index} />)
  }
  ...
  return (
    <div className="p-4 sm:p-6 pt-6">
      ...
      {hasSearched && !isLoading && !isTyping && (
        <div className="max-w-2xl mx-auto mb-4">
          <p className="text-gray-600">
            {memes.length === 0
              ? `No results found for "${searchQuery}"`
              : `Showing ${memes.length} result${memes.length === 1 ? "" : "s"} for "${searchQuery}"`}
          </p>
        </div>
      )}
      ...
      <div className="w-full mx-auto px-2 sm:px-4">
        {isLoading || isTyping ? (
          // Show skeleton loading in masonry grid
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-800 mb-2">No memes found</h3>
            <p className="text-gray-500 max-w-md">
              We couldn't find any memes matching your search. Try using different keywords or browse our trending
              memes.
            </p>
            <button
              onClick={handleClearSearch}
            ...
            {memes.length === 0 && !hasSearched && (
              <div className="col-span-full text-center py-8 sm:py-12 w-full">
                <div className="max-w-md mx-auto">
                  ...
                  <p className="text-gray-500">Search for memes or browse our collection!</p>
                </div>
              </div>
            )}
          </TrueMasonryGrid>
