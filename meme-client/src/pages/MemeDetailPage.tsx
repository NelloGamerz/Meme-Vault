...
// import { Heart, Bookmark, Download, Share2, MessageCircle, Send, ArrowLeft, User, X, Play, Pause, Volume2, VolumeX } from "lucide-react";
...
//     const allMemes = [...memes, ...userMemes, ...likedMemes, ...savedMemes];
//     const foundMeme = allMemes.find((m) => m.id === id);
//     return foundMeme ? { ...foundMeme, comments: foundMeme.comments || [] } : null;
//   });
...
//         if (fetchedMeme) {
//           setMeme({ ...fetchedMeme, comments: fetchedMeme.comments || [] });
//           if (user.userId) {
...
//     initializePage();
//   }, [id, fetchMemeById, navigate, fetchUserProfile, user.userId]);
...
//       setComment("");
//       const updatedMeme = await fetchMemeById(id);
//       if (updatedMeme) {
//         setMeme({ ...updatedMeme, comments: updatedMeme.comments || [] });
//       }
...
//       // Refresh meme data to get updated like count
//       const updatedMeme = await fetchMemeById(meme.id);
//       if (updatedMeme) {
//         setMeme({ ...updatedMeme, comments: updatedMeme.comments || [] });
//       }
//       // Refresh user data to update liked memes list
//       await fetchUserProfile(user.userId);
//     }
//   };
...
//       const updateMeme = await fetchMemeById(meme.id);
//       if (updateMeme) {
//         setMeme({...updateMeme, comments: updateMeme.comments || [] });
//       }
//       // Refresh user data to update saved memes list
//       await fetchUserProfile(user.userId);
//     }
//   };
...
export const MemeDetailPage: React.FC = () => {
    ...
    const initializePage = async () => {
      if (id) {
        ...
        if (fetchedMeme) {
          ...
          // Connect to WebSocket for real-time comments when the meme is loaded
  ...
  }, [id, fetchMemeById, navigate, fetchUserProfile, user.userId, connectToWebSocket, disconnectWebSocket])

  // Listen for updates to memes state
  useEffect(() => {
    if (id) {
      const allMemes = [...memes, ...userMemes, ...likedMemes, ...savedMemes]
      const updatedMeme = allMemes.find((m) => m.id === id)
      if (updatedMeme && JSON.stringify(updatedMeme) !== JSON.stringify(meme)) {
        setMeme({ ...updatedMeme, comments: updatedMeme.comments || [] })
      }
    }
  }, [id, memes, userMemes, likedMemes, savedMemes, meme])
  ...
  const isLiked = likedMemes.some((m) => m.id === meme.id)
  const alreadySaved = savedMemes.some((m) => m.id === meme.id)
  const commentCount = meme.comments?.length || 0
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    e.preventDefault()
    if (comment.trim() && id) {
      // Check if WebSocket is connected before submitting
            ...
            <div className="relative bg-gray-900 flex items-center justify-center">
              ...
              <div className="lg:hidden absolute bottom-4 right-4 flex gap-2">
                <button
                  ...
                  aria-label={alreadySaved ? "Unsave" : "Save"}
                >
                  <Bookmark className={cn("w-5 h-5", alreadySaved && "fill-current")} />
                </button>
              </div>
            </div>
